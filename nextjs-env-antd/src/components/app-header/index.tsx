...
const AppHeader = memo(function MyAppHeader(props: any) {
  ...
  // useSelector 用于从 Redux store 中提取数据
  // 接受函数作为参数 其中入参是整个 Redux store 的状态 返回userInfo的信息
  const { userInfo } = useSelector((state: any) => {
    return {
      // getIn 方法 是Immutable.js 提供的方法, 用于访问嵌套的对象属性
      // getIn 方法更加的安全, 当路径中某个属性不存在时不会抛出错误,而是返回 undefined
