<template>
  <div>
    ...
    <el-dialog
      :close-on-click-modal="false"
      :before-close="randomAddDialogClose"
      :visible="showRandomAddDialog"
      :title="'随机添加'"
      width="400px"
    >
      <el-form
      ...
      >
        ...
        >
          <el-input-number
            v-model.number="random.quantity"
            :min="1"
            :max="9999"
            :precision="0"
            style="width: 100%;"
  ...
  methods: {
    ...
    doRandomAdd() {
      this.$refs.random.validate((valid) => {
        if (valid) {
          ...
          axios.post('/api/fl/randomAdd/' + this.random.quantity).then(res => {
            const {data} = res
            if (data.success) {
    ...
    doAdd(){
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          axios.post('/api/fl/save', this.form).then(res => {
            const {data} = res
            if (data.success) {
    ...
    doEdit(){
      this.$refs.form.validate((valid) => {
        if (valid) {
          this.buttonLoading = true
          axios.post('/api/fl/update', this.form).then(res => {
            const {data} = res
            if (data.success) {
    ...
    doDelete(row) {
      this.buttonLoading = true
      axios.post('/api/fl/delete/' + row.id).then(res => {
        const {data} = res
        if (data.success) {
    ...
    doBatchDelete(rows) {
      axios.post('/api/fl/delete', rows.map(e => e.id)).then(res => {
        const {data} = res
        if (data.success) {
          this.$message.success('删除成功')
          this.buttonLoading = false
    ...
    searchList() {
      ...
      axios.post('/api/fl/searchList', params).then(res => {
        const {data} = res
        if (data.success) {
