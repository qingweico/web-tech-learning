 ...
 */
package cn.qingweico.common.util.excel.export;
...
import cn.qingweico.common.util.excel.entity.enums.ExcelExportEnum;
import cn.qingweico.common.util.excel.entity.enums.ExcelType;
import cn.qingweico.common.util.excel.entity.params.ExportParams;
import cn.qingweico.common.util.excel.exception.ExcelExportException;
import cn.qingweico.common.util.excel.export.base.BaseExportService;
import cn.qingweico.common.util.excel.export.styler.IExcelExportStyler;
import cn.qingweico.common.util.excel.util.PoiExcelGraphDataUtil;
import cn.qingweico.common.util.excel.util.PoiMergeCellUtil;
...
public class ExcelExportService extends BaseExportService {
    ...
    public void createSheet(Workbook workbook, ExportParams entity, Class<?> pojoClass,
        ...
        try {
            ...
            List inclusionsList = entity.getInclusions() == null ? Lists.newArrayList() : Arrays.asList(entity.getInclusions());

            Set<String> exclusions = entity.getExclusions() == null ? Sets.newHashSet() : Sets.newHashSet(entity.getExclusions());
            if (!CollectionUtils.isEmpty(inclusionsList)) {
                Arrays.stream(fileds).forEach(f -> {
                    if (!inclusionsList.contains(f.getName()) && f.getAnnotation(Excel.class) != null) {
                        exclusions.add(f.getAnnotation(Excel.class).name());
                    }
                });
            }
            entity.setExclusions(exclusions.toArray(new String[exclusions.size()]));
            if (CollectionUtils.isEmpty(inclusionsList)) {
                getAllExcelField(entity.getExclusions(), targetId, fileds, excelParams, pojoClass,
                        null, null);
            } else {
                inclusionsList.forEach(i -> Arrays.stream(fileds).forEach(f -> {
                    if (i.equals(f.getName()) && f.getAnnotation(Excel.class) != null) {
                        Excel excel = f.getAnnotation(Excel.class);
                        String name = PoiPublicUtil.getValueByTargetId(excel.name(), targetId, null);
                        if (StringUtils.isNotBlank(name)) {
                            try {
                                excelParams.add(createExcelExportEntity(f, targetId, pojoClass, null, null));
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
