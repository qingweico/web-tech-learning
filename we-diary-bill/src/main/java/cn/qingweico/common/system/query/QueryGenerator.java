package cn.qingweico.common.system.query;
...
import cn.qingweico.common.system.util.JeecgDataAutorUtils;
...
import cn.qingweico.common.util.SqlInjectionUtil;
...
public class QueryGenerator {
    ...
    private static final String COLUMN_SPLIT = "__";
    private static final String NOT_EQUAL = "!";

    private static final ThreadLocal<Map<String, String>> THREAD_LOCAL_COLUMN_MULTI = new TransmittableThreadLocal<Map<String, String>>() {
        @Override
    ...
    public static <T> QueryWrapper<T> initQueryWrapper(T searchObj, Map<String, String[]> parameterMap) {
        ...
        parameterMap.forEach((k, v) -> {
            if (k.contains(COLUMN_SPLIT)) {
                log.info("包含列名称: {}--> {}", k, v[0]);
                THREAD_LOCAL_COLUMN_MULTI.set(ImmutableMap.of(k, v[0]));
//                Arrays.stream(k.split(COLUMN_SPLIT)).forEach(i -> {
//                    ReflectUtils.setFieldValue(searchObj, i, v[0]);
//                });
            }
        });
    ...
    public static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap) {
		...
		*/
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            handelQueryParam(queryWrapper, searchObj, parameterMap, name, type, ruleMap);
        }

        Map<String, String> localMap = THREAD_LOCAL_COLUMN_MULTI.get();
        THREAD_LOCAL_COLUMN_MULTI.remove();
        if (!CollectionUtils.isEmpty(localMap)) {
            queryWrapper.and(t -> localMap.forEach((k, v) -> {
                QueryRuleEnum rule = convert2Rule(v);
                if (rule == null || QueryRuleEnum.EQ.equals(rule)) {
                    rule = QueryRuleEnum.EQ;
                }
                Object finalValue = replaceValue(rule, v);
                QueryRuleEnum finalRule = rule;
                Arrays.stream(k.split(COLUMN_SPLIT)).forEach(i -> {
                    t.or();
                    addEasyQuery(t, i, finalRule, finalValue);
                });
            }));
        }
    ...
    private static void handelQueryParam(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap, String name, Class typeCls, Map<String, SysPermissionDataRule> ruleMap) {
        try {

            if (judgedIsUselessField(name) || !PropertyUtils.isReadable(searchObj, name)) {
                return;
            }
            Field originField = ReflectUtils.getAccessibleField(searchObj, name);
            TableField tableField = originField.getAnnotation(TableField.class);
//            if (tableField != null && !tableField.exist()) {
//                return;
//            }

            String type = typeCls.toString();
            // 添加 判断是否有区间值
        ...
        if (!"".equals(value)) {
            ...
            switch (type) {
                ...
                case "class java.lang.String":
                    temp = DateUtil.isValidDate(value) ? getDateQueryByRule(value, rule) : value;
                    break;
                default:
                    temp = value;
                    break;
...
//		log.info("--查询规则-->"+name+" "+rule.getValue()+" "+value);
    ...
    public static Map<String, SysPermissionDataRule> getRuleMap() {
        Map<String, SysPermissionDataRule> ruleMap = new HashMap<>(16);
