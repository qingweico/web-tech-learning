...
import cn.qingweico.modules.system.entity.SysRole;
...
import cn.qingweico.modules.system.service.ISysRoleService;
...
/*		for (int i = 0; i < list.size(); i++) {
			...
			if (StringUtils.isEmpty(list.get(i).getSignature())) {
				...
				try {
					if (!RASUtil.verifySign(list.get(i).getSignature(), list.get(i).getRoleName())) {
						temp.add(list.get(i));
					}
				} catch (Exception e) {
					temp.add(list.get(i));
					log.error("查询用户角色签名校验错误---> {}", e.getMessage());
				}
			}
	...
	@OperationLogDetail(detail ="角色管理, 添加",level= LogLevelType.ONE,operationType = OperationType.INSERT,operationUnit = OperationUnit.USER)
	public Result<SysRole> add(@RequestBody SysRole role) {
		Result<SysRole> result = new Result<SysRole>();
		try {
			role.setCreated(new Date());
			role.setSignature(RASUtil.sign(role.getRoleName()));
			sysRoleService.save(role);
			result.success("添加成功！");
		} catch (Exception e) {
			log.error("添加用户角色错误--> {}", e.getMessage());
	...
	@OperationLogDetail(detail ="角色管理, 编辑",level= LogLevelType.ONE,operationType = OperationType.UPDATE,operationUnit = OperationUnit.USER)
	public Result<SysRole> edit(@RequestBody SysRole role) {
		...
		try {
			...
			log.error("编辑用户角色错误---> {}", e.getMessage());
		}
		return result;
	}
	...
	@OperationLogDetail(detail ="角色管理, 删除",level= LogLevelType.ONE,operationType = OperationType.DELETE,operationUnit = OperationUnit.USER)
	 ...
	 */
	@CacheEvict(value = CacheConstant.LOGIN_USER_RULES_CACHE, allEntries = true)
	@OperationLogDetail(detail = "角色管理, 批量删除", level = LogLevelType.ONE, operationType = OperationType.DELETE, operationUnit = OperationUnit.USER)
	...
	@OperationLogDetail(detail ="角色管理, 保存数据规则至角色菜单关联表",level= LogLevelType.ONE,operationType = OperationType.INSERT,operationUnit = OperationUnit.USER)
